"""
test_state_encoder.py - Unit tests for the state encoder module.
...
def test_single_card_in_tableau():
    ...
    # Check that the card is encoded in the tableau section
    # The first tableau pile should have the card encoded
    # This is a basic check - more detailed validation would require
    # understanding the exact encoding scheme
...
def test_stock_count():
    ...
    # The stock count should be encoded (normalized value)
    # This is a basic check - exact position depends on encoding scheme
    assert not np.allclose(state, np.zeros_like(state))

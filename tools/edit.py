...
class EditTool(BaseTool):
        ...
        **kwargs,
        ...
        try:
            if cmd_enum is Command.CREATE:
                ...
                result = await self._cmd_str_replace_enhanced(_path, old_str, new_str, match_mode)
            elif cmd_enum is Command.INSERT:
                result = await self._cmd_insert_enhanced(_path, insert_line, new_str)
    ...
    async def _cmd_str_replace_enhanced(
        self, path: Path, old_desc: Optional[str], new_desc: Optional[str], mode: str
    ) -> ToolResult:
        """Enhanced string replacement with LLM support."""
        if mode == "llm":
            return await self._cmd_str_replace_llm(path, old_desc, new_desc)
        else:
            # Fall back to traditional replacement for non-LLM modes
            return self._cmd_str_replace(path, old_desc, new_desc, mode)

    async def _cmd_str_replace_llm(
        self, path: Path, old_desc: Optional[str], new_desc: Optional[str]
    ) -> ToolResult:
    ...
    async def _cmd_insert_enhanced(
        self, path: Path, line: Optional[int], instruction: Optional[str]
    ) -> ToolResult:
        """Enhanced insertion with LLM support."""
        if line is None or instruction is None:
            raise ToolError("insert_line and new_str required for insert")
        
        # Check if instruction looks like a natural language description
        if len(instruction) > 50 or any(word in instruction.lower() for word in ['add', 'insert', 'create', 'implement', 'function', 'class', 'method']):
            return await self._cmd_insert_llm(path, line, instruction)
        else:
            # Use traditional insert for simple text
            return self._file_insert(path, line, instruction)

    async def _cmd_insert_llm(
        self, path: Path, line: int, instruction: str
    ) -> ToolResult:

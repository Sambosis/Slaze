...
class WebNavigatorTool:
    ...
    description: str = """A comprehensive tool that uses Playwright to perform various web interactions such as reading information, 
    ...
    async def __call__(
        ...
        url: str,
        action: Literal[
            "read", "navigate", "download", "fill_form", "extract_data", "click_element"
        ],
        ...
        try:
            ...
            if action == "read":
                ...
                if not file_path:
                    raise ValueError(
                        "Parameter `file_path` is required for action: download"
                    )
                ...
                if not form_selector or not form_data:
                    raise ValueError(
                        "Parameters `form_selector` and `form_data` are required for action: fill_form"
                    )
                ...
                if not data_selector:
                    raise ValueError(
                        "Parameter `data_selector` is required for action: extract_data"
                    )
                ...
                if not element_selector:
                    raise ValueError(
                        "Parameter `element_selector` is required for action: click_element"
                    )
    ...
    async def read_info(
        ...
        for element in soup.find_all(
            class_=lambda x: x and any(track in x.lower() for track in tracking_classes)
        ):
            element.decompose()
    ...
    async def fill_form(
        self, page, url: str, form_selector: str, form_data: Dict[str, str]
    ) -> str:
        """
    ...
    def integrate_external_api(
        self, api_url: str, params: Optional[Dict[str, Any]] = None
    ) -> Any:
        """
        ...
        if response.status_code == 200:
            ...
            raise ConnectionError(
                f"Failed to connect to external API '{api_url}'. Status code: {response.status_code}"
            )

...
class WriteCodeTool(BaseAnthropicTool):
    ...
    description: str = (
        "Generates a codebase consisting of up to 5 files based on descriptions and import lists. "
        "This is the tool to use to generate code files for a codebase, and it can create full or partial implementations up to 5 files at a time. "
        "Use this tool to generate init files while writing the full code asynchronously to the host filesystem."
    )
            ...
            if file_details:  # Only proceed if there are code files to process
                if self.display:
                    ...
                    console_output = self._format_terminal_output(
                        ...
                        result="Starting code generation process (generating files)",
                ...
                code_results = await asyncio.gather(
                    *code_gen_tasks, return_exceptions=True
                )
    ...
    def extract_code_block(
        ...
        if start_marker == -1:
            ...
            try:
                ...
                if language in {"text", "plaintext", "plain"}:
                    language = "unknown"
